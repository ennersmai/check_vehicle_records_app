  ...
  <div class="min-h-screen bg-white pb-36">
    ...
    <div class="px-10 mt-8">
      ...
      <div v-if="!user" class="text-center py-12">
        ...
        <p class="text-gray-500 mb-4">Log in and your vehicles will appear here</p>
        <button @click="$router.push('/login?redirect=/garage')" class="text-primary font-medium">Log In</button>
      </div>
      ...
      <div v-else-if="user" class="space-y-4">
        ...
        >
          ...
          <div class="flex gap-2">
            ...
            <button 
              @click="viewBasicInfo(vehicle.vrm)"
        ...
        <div v-if="garageVehicles.length === 0" class="text-center py-12">
          ...
          <p class="text-gray-400 text-sm mb-4">Your vehicle lookups will appear here</p>
          <button 
            @click="addVehicle"
...
// Fetch ALL user's lookups (basic + premium) as their "garage" vehicles
const fetchGarageVehicles = async () => {
  if (!user.value) return;
  
  loading.value = true;
  try {
    // Get all vehicle lookups for the current user
    ...
    // Also get premium lookups to know which ones have premium
    ...
    // Create a set of VRMs that have premium
...
const viewBasicInfo = (vrm: string) => {
  router.push(`/confirm-vehicle/${vrm.replace(/\s/g, '')}`);
};
...
const deleteVehicle = async () => {
  ...
  deleting.value = true;
  try {
    // Delete from vehicle_lookups (this removes the vehicle from garage)
