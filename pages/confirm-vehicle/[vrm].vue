  ...
  try {
    ...
    if (cachedData) {
      ...
      if (cachedData.registrationNumber) {
        vehicleData.value = cachedData.tax ? mapCheckCarDetailsBasic(cachedData) : mapDvlaData(cachedData);
      } else {
        vehicleData.value = cachedData;
      }
      // Keep raw data for fields not in mapped data
      vehicleData.value = { ...vehicleData.value, ...cachedData };
    } else {
